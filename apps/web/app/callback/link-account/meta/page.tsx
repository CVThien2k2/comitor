...
const META_OAUTH_CLIENT_ID = process.env.META_APP_ID || ""
  ...
  useEffect(() => {
    ...
    const handleCallback = async () => {
      try {
        ...
        if (error) {
          const errorMsg = errorDescription || error || "Đã xảy ra lỗi khi xác thực"
          setState("error")
        ...
        if (!code) {
          ...
          setErrorDetails("Không tìm thấy mã xác thực trong URL. Vui lòng thử lại.")
          toast.error("Thiếu mã xác thực", { position: "bottom-right" })
          return
        }

        const response = await axios.post(
          `${BASE_URL}/link-accounts/meta`,
          {
            code,
          },
          {
        ...
        if (response.status !== 200) {
          ...
          const errorMsg = errorData.message || "Không thể kết nối tài khoản Meta/Facebook"
          setState("error")
        ...
        setState("success")
        setMessage(data.message || "Đã kết nối tài khoản Meta/Facebook thành công!")
      ...
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="text-center">
          <div className="mb-4 flex justify-center">
            {state === "loading" && <Icons.spinner className="h-16 w-16 animate-spin text-blue-500" />}
            {state === "success" && <Icons.checkCircle2 className="h-16 w-16 text-emerald-500" />}
            {state === "error" && <Icons.xCircle className="h-16 w-16 text-red-500" />}
          </div>
        ...
        <CardContent>
          {state === "loading" && (
            <div className="space-y-4 text-center">
              <p className="text-sm text-muted-foreground">Đang kết nối tài khoản Meta/Facebook của bạn...</p>
            </div>
          )}
            ...
            <div className="space-y-4 text-center">
              <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-700 dark:bg-emerald-900/30">
                <p className="text-sm text-emerald-800 dark:text-emerald-100">
                  Tài khoản Meta/Facebook đã được kết nối thành công. Bạn sẽ được chuyển hướng trong giây lát...
                </p>
              </div>
            </div>
          )}
...
export default function MetaCallbackPage() {
  return (
    <Suspense
      fallback={
        <div className="flex min-h-screen items-center justify-center bg-background p-4">
          <Icons.spinner className="h-16 w-16 animate-spin text-blue-500" />
        </div>
      }
    >
