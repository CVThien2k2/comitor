...
export type PermissionCode = (typeof PERMISSION_DEFINITIONS)[PermissionKey]["code"]
export type Permission = PermissionCode

export const P = Object.fromEntries(Object.entries(PERMISSION_DEFINITIONS).map(([key, val]) => [key, val.code])) as {
  [K in PermissionKey]: (typeof PERMISSION_DEFINITIONS)[K]["code"]
}

export const ALL_PERMISSIONS: PermissionCode[] = Object.values(PERMISSION_DEFINITIONS).map((p) => p.code)
...
export const DEFAULT_PERMISSIONS: DefaultPermission[] = Object.entries(PERMISSION_DEFINITIONS).map(([key, val]) => ({
