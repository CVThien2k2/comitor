...
import { ZaloOaProfileResponse } from "./types"
...
export function mapProfileToGoldenProfile(
  response: ZaloOaProfileResponse,
  userId: string
): Partial<GoldenProfile> {
  const profile = response.data ?? {}

  const id =
    readString(profile.id) ?? readString(profile.user_id) ?? userId
  ...
  const gender = mapGender(
    profile.user_gender ?? profile.gender ?? profile.sex
  )

  const dateOfBirth = parseDate(
    profile.birth_date ?? profile.birthday ?? profile.date_of_birth
  )

  const primaryPhone =
    readString(profile.phone) ??
    readString(profile.user_phone) ??
    readString(profile.phone_number)
