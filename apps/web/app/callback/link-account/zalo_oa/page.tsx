    ...
    const handleCallback = async () => {
      try {
        ...
        const response = await axios.post(
          `${BASE_URL}/link-accounts/zalo-oa`,
          {
            code,
          },
          {
        ...
        if (response.status !== 201) {
          ...
          const errorMsg = errorData.message || "Không thể kết nối tài khoản Zalo OA"
          setState("error")
        ...
        <CardContent>
          {state === "loading" && (
            <div className="space-y-4 text-center">
              <p className="text-sm text-muted-foreground">Đang kết nối tài khoản Zalo OA của bạn...</p>
            </div>
          )}
            ...
            <div className="space-y-4 text-center">
              <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-700 dark:bg-emerald-900/30">
                <p className="text-sm text-emerald-800 dark:text-emerald-100">
                  Tài khoản Zalo OA đã được kết nối thành công. Bạn sẽ được chuyển hướng trong giây lát...
                </p>
              </div>
            </div>
          )}
            ...
            <div className="flex w-full gap-2">
              ...
              <Button
                onClick={() => {
                  ...
                  const redirectUrl = process.env.NEXT_PUBLIC_ZALO_OA_REQUEST_PERMISSION_APP_URL || "/"
